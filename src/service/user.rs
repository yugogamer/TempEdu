...
pub async fn add_user(conn : &Client, user : &UserInsertion) -> Result<User, Box<dyn Error>>{
    let row = conn.query_one("INSERT INTO accounts (username, first_name, last_name, abreviate_name, mail) VALUES ($1, $2, $3, $4, $5)", &[&user.username, &user.first_name, &user.last_name, &user.abreviate_name, &user.mail]).await?;
